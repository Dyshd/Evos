...
export default function PopularDishes() {
  const { popularDishes } = useSelector(popularDishesRetriever);
  const fixedSlides: Product[] = Array.isArray(popularDishes) ? popularDishes : [];
        ...
        <Stack className="popular-section">
          ...
          <div className="cards-frame">
            {fixedSlides.length > 0 ? (
              ...
              >
                {fixedSlides.map((product: Product, index: number) => {
                  const firstImage = product?.productImages?.[0];
                  const imagePath = firstImage
                    ? `${serverApi}/${firstImage}`
                    : "/img/default-dish.png";

                  return (
                    <SwiperSlide key={product?._id || index}>
                      <CssVarsProvider>
                        <Card className="popular-card" variant="plain">
                          {/* Rasm qismi */}
                          <Box className="card-img-wrapper">
                            ...
                            <Box className="view-badge">
                              {product?.productViews ?? 0}
                              <VisibilityIcon sx={{ fontSize: "14px", ml: 0.5 }} />
                            </Box>
                          </Box>

                          {/* Ma'lumot qismi */}
                          <CardContent className="card-info-content">
                            <Typography level="title-lg" className="dish-name">
                              {product?.productName || "Nomsiz taom"}
                            </Typography>

                            <Typography level="body-sm" className="dish-desc">
                              <DescriptionOutlinedIcon sx={{ fontSize: "16px", mr: 1 }} />
                              {product?.productDesc || "Ta'rif mavjud emas"}
                            </Typography>

                            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mt: 2 }}>
                              <Typography className="dish-price">
                                {product?.productPrice?.toLocaleString() ?? 0} <span>so'm</span>
                              </Typography>
                              <button className="order-btn-small">Order</button>
                            </Stack>
                          </CardContent>
                        </Card>
                      </CssVarsProvider>
              ...
              <Box className="no-data">Hozircha ommabop taomlar mavjud emas!</Box>
            )}
          </div>
        </Stack>
      </Container>
